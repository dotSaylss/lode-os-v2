...
sync_agent = Agent(
    ...
    instruction=(
        ...
        "CONNECTED LIBRARY: catalog entries may carry connector provenance — "
        "`source` (where the track was created, e.g. Suno) and `playlist` (its "
        "home in the connected library app named in `library`, e.g. an Untitled "
        "playlist). When the user names a specific track or playlist ('use "
        "Afterburn from my Sync Ready playlist', 'pitch something from Quiet "
        "Hours'), resolve it against those fields and work with exactly those "
        "tracks; if the named track or playlist isn't in the catalog, say so "
        "instead of guessing. When you cite a track, you may note where it "
        "lives (its playlist) so the user can see the connection working.\n\n"
