<script lang="ts">
	...
	import { MARKETPLACE, categoryLabel } from '$lib/marketplace.config';
	...
	let messages = $state<Message[]>([
		{
			...
			role: 'agent',
			content:
				`I'm the ${MARKETPLACE.matchmakerName}. Describe your song and what it needs: mixing, ` +
				"mastering, cover art, vocals, sync, video, promo. I'll assemble the right team of vetted " +
				'providers, explain why each fits, and propose how splits and rights get routed.'
		}
	]);
...
<div class="chat-card">
	<div class="chat-head">
		...
		<div class="chat-head-meta">
			<b>{MARKETPLACE.matchmakerName}</b>
			<span><span class="chat-live"></span> {MARKETPLACE.matchmakerTagline}</span>
		</div>
							...
							<div class="ev-list">
								{#each ev.providers as p (p.name)}
									<div class="ev-prov">
										...
										{/if}
										{#if p.category}
											<span class="ev-cat">{categoryLabel(p.category)}</span>
										{/if}
										<span class="ev-meta">
											{#if p.rating}
