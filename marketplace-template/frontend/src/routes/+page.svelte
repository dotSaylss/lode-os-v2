<script lang="ts">
	...
	import { MARKETPLACE, categoryLabel, categoryTone, avatarTone } from '$lib/marketplace.config';
	...
	// Category filter behind a single funnel button — one quiet control.
	...
	// a brief is submitted — then it reveals full-width in place.
	...
	// Card-level entry into the same grounded matchmaker: one tap turns a listing
	// into a real agent consultation with cited evidence.
	async function askAbout(p: Provider) {
		await revealPanel();
		if (!chat) await tick();
		chat?.send(
			`Tell me about ${p.name} for ${categoryLabel(p.category).toLowerCase()}: are they the right fit for my project, and what would it cost?`
		);
	}
</script>
...
<div class="v3-stage-wide">
	<header class="v3-header">
		<div>
			<span class="v3-date">{MARKETPLACE.eyebrow}</span>
			<h1>{MARKETPLACE.title}</h1>
			<p class="v3-vp">{MARKETPLACE.subtitle}</p>
		</div>
	...
	<div class="svc-stack">
		...
		<section class="svc-card svc-brief">
			<span class="eyebrow">{MARKETPLACE.briefLabel}</span>
			<form
				onsubmit={(e) => {
					e.preventDefault();
					runBrief();
			...
			>
				<textarea bind:value={brief} rows="3" placeholder={MARKETPLACE.briefPlaceholder}></textarea>
				<div class="svc-brief-foot">
					<button type="submit" class="v3-act" disabled={!brief.trim()}>
						{MARKETPLACE.briefCta} <Icon name="arrow-right" size={16} color="#fff" />
					</button>
				</div>
			</form>
		</section>
			...
			<div class="svc-card-head">
				...
				<div class="svc-toolbar">
					...
					{#if activeCategory !== 'all'}
						...
						>
							{categoryLabel(activeCategory)} <Icon name="x" size={12} />
						</button>
					{/if}
						...
						{#if filterOpen}
							<div class="svc-filter-pop" role="menu" transition:fade={{ duration: 120 }}>
								{#each categories as cat}
									...
									>
										<span>{cat === 'all' ? 'All categories' : categoryLabel(cat)}</span>
					...
					{#each visibleProviders as p (p.id)}
						<div class="svc-prov">
							<div class="svc-prov-top">
								<span class="svc-avatar {avatarTone(p.category)}">{initials(p.name)}</span>
								<div class="svc-prov-id">
									...
									<span class="svc-tag {categoryTone(p.category)}">{categoryLabel(p.category)}</span>
								</div>
							...
							<div class="svc-prov-foot">
								...
								<button
									class="svc-ask"
									type="button"
									title="Ask the Matchmaker about {p.name}"
									aria-label="Ask the Matchmaker about {p.name}"
									onclick={() => askAbout(p)}
								>
